...
1. Описать модель `User` на уровне концепции:
  ...
  - Типы данных
  - Какие поля вводятся в ручную, а какие из API
Псс, не забудь про `ID`, тебе же надо как-то различать `User'ов`?  
**Что изучить / закрепить:**
...
- ✅ **Задание 1 — Установить и настроить PostgreSQL**
...
- ✅ **Задание 2 — Изучить типы данных PostgreSQL**
1. 🎯 **Цель**: понять, какие типы подойдут под структуру User.
2. 📌 **Что сделать**:
  - Напиши таблицу соответствия между полями `User` и типами **PostgreSQL**
🧩 **Подсказка**:  `VARCHAR`, `INTEGER`... (какой куда применить? OK, Google?)

- ✅ **Задание 3 — Написать SQL-миграцию вручную**
1. 🎯 **Цель**: описать SQL-команду для создания таблицы `users`.
2. 📌 **Что сделать**:
  - Напиши SQL-запрос `CREATE TABLE users (...)` для своей модели
  - Добавь поля, ограничения (`NOT NULL`, `PRIMARY KEY`, `UNIQUE`, если нужно)
  - (опционально) — создай файл миграции `001_create_users.sql`

- ✅ **Задание 4 — Настроить подключение к PostgreSQL из Go**
1. 🎯 **Цель**: научиться подключать приложение к базе.
2. 📌 **Что сделать**:
  - Выбери библиотеку: `pgx`, `gorm`, `sqlx`, стандартный `database/sql`
  - Настрой подключение к БД через переменные окружения или config-файл
  - Проверь, что соединение успешно (ping)

- ✅ **Задание 5 — Использовать миграции через инструмент**
1. 🎯 **Цель**: автоматизировать работу со схемой БД.
2. 📌 **Что сделать**:
  - Выбери и установи библиотеку миграций:
    - `golang-migrate` (CLI + SQL-файлы)
    - `GORM AutoMigrate()` (если используешь GORM)
  - Создай первую миграцию (либо .sql, либо программно)
  - Примените миграцию к базе и проверь, что таблица создана
